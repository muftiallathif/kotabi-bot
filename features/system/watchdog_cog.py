...
# Kontrol eksplisit: apakah file cog BARU (features/<fitur>/*_cog.py) boleh
# otomatis di-load tanpa restart manual. Default FALSE — file baru harus
# dimuat lewat restart bot (jalur deploy resmi/CI-CD), bukan otomatis oleh
# watchdog. Reload cog yang SUDAH ter-load (saat file-nya diedit) tetap
...
class Watchdog(commands.Cog):
    def __init__(self, bot: KotabiBot):
        self.bot = bot
        self.features_folder = Path(bot.features_folder)
    ...
    def _iter_cog_files(self):
        """
        Scan features/<nama_fitur>/*_cog.py, satu level saja di dalam tiap
        folder fitur, TIDAK masuk ke sub-folder support/ (isinya bukan Cog).
        Mengikuti pola KotabiBot._discover_cog_modules() di core/bot.py.
        """
        if not self.features_folder.exists():
            return
        for feature_dir in sorted(self.features_folder.iterdir()):
            if not feature_dir.is_dir() or feature_dir.name.startswith("_"):
                continue
            for cog_file in sorted(feature_dir.glob("*_cog.py")):
                yield feature_dir, cog_file

    def _snapshot_mtimes(self):
        for feature_dir, path in self._iter_cog_files():
            module = f"{self.features_folder.name}.{feature_dir.name}.{path.stem}"
            self._mtimes[module] = path.stat().st_mtime

    def _module_to_path(self, module: str) -> Path:
        # module contoh: "features.social.bookmark_cog"
        parts = module.split(".")
        feature_name, stem = parts[-2], parts[-1]
        return self.features_folder / feature_name / f"{stem}.py"
