...
SELFMUTE_SETTINGS_PATH = os.getenv("ALT_SELFMUTE_SETTINGS_PATH") or "features/moderation/selfmute_settings.yml"
...
GET_ALL_MUTES_QUERY = """
SELECT * FROM active_mutes WHERE guild_id = ? ORDER BY end_time ASC
"""

GET_USER_MUTE_QUERY = """
SELECT guild_id, user_id, mute_role_id, roles_to_restore, end_time
FROM active_mutes WHERE guild_id = ? AND user_id = ?
"""

GET_ALL_USER_MUTES_QUERY = """
SELECT guild_id, user_id, mute_role_id, roles_to_restore, end_time
FROM active_mutes WHERE user_id = ?
"""
...
class Selfmute(commands.Cog):
    ...
    async def perform_mute(self, member: discord.Member, mute_role: discord.Role, unmute_time: datetime):
        roles_not_to_remove = [
            ...
            for role_id in selfmute_settings.get(member.guild.id, {}).get("roles_not_to_remove", [])
        ]
        roles_to_save = [
            ...
            and role not in roles_not_to_remove
        ]
    ...
    async def unmute_user(self, interaction: discord.Interaction, member: discord.Member):
        ...
        if mute_data:
            await interaction.followup.send(
                Msg.mute_unmuted_by_admin(member.mention),
                ephemeral=True
            )
    ...
    async def perform_user_unmute(self, member: discord.Member, channel: discord.TextChannel, mute_data):
        ...
        all_selfmute_roles = [member.guild.get_role(role_id) for role_id in all_self_mute_role_ids]
        await member.edit(roles=[role for role in member.roles if role not in all_selfmute_roles])
        ...
        if role_ids_to_restore:
            ...
            roles_to_restore = [
                role for role in roles_to_restore
                if not role.is_default() and not role.is_premium_subscriber() and role.is_assignable()
            ]
            await member.add_roles(*roles_to_restore)
            if channel:
                await channel.send(
                    Msg.mute_roles_restored(member.mention, ', '.join([role.mention for role in roles_to_restore])),
    ...
    async def selfmute(self, interaction: discord.Interaction, hours: Optional[int] = 0, minutes: Optional[int] = 0):
        ...
        if hours < 0 or minutes < 0:
            await interaction.followup.send(Msg.MUTE_NEGATIVE_DURATION, ephemeral=True)
            return
        if hours > 721:
            await interaction.followup.send(Msg.MUTE_TOO_LONG, ephemeral=True)
            return

        all_self_mute_role_ids = selfmute_settings.get(interaction.guild.id, {}).get("mute_roles", [])
        all_selfmute_roles = [interaction.guild.get_role(role_id) for role_id in all_self_mute_role_ids]

        if not all_selfmute_roles:
            await interaction.followup.send(
                Msg.MUTE_NO_ROLE,
                ephemeral=True
            )
        ...
        if any(role in interaction.user.roles for role in all_selfmute_roles):
            await interaction.followup.send(Msg.MUTE_ALREADY_MUTED, ephemeral=True)
            return

        allowed_roles = [
            interaction.guild.get_role(role_id)
            for role_id in selfmute_settings.get(interaction.guild.id, {}).get("allowed_ids", [])
        ]
        if not any(role in interaction.user.roles for role in allowed_roles):
            allowed_mention = ", ".join([role.mention for role in allowed_roles])
            await interaction.followup.send(
                Msg.mute_not_allowed(allowed_mention),
                ephemeral=True
            )
            return
        ...
        async def mute_callback(interaction: discord.Interaction):
            ...
            mute_message = Msg.mute_applied(interaction.user.mention, mute_role.name, int(unmute_time.timestamp()))
            role_message = Msg.mute_previous_roles(', '.join([role.mention for role in user_roles]))
        ...
        my_select = discord.ui.Select(placeholder=Msg.MUTE_SELECT_PLACEHOLDER)
        for role in all_selfmute_roles:
            my_select.add_option(label=role.name, value=str(role.id))

        my_view.add_item(my_select)
        my_select.callback = mute_callback
        await interaction.followup.send(Msg.MUTE_SELECT_ROLE, view=my_view, ephemeral=True)
